...
# 0. Demo data -----------------------------------------------
# You should get in habit of always viewing data first
# View(mtcars)
mtcars <- mtcars %>% 
  mutate(ID = 1:n())
mtcars_train <- mtcars %>% 
...
# 1. Fit a model ----------------------------------------
model_formula <- as.formula("mpg ~ hp + cyl")
model <- lm(model_formula, data=mtcars_train)
...
# 2. Make predictions ----------------------------------------
# One way: Note the output is a vector
predict(model, newdata=mtcars_test)

# The other way. Note the output is a data frame with
# -all the original data
# -new variable start with a period
model %>% 
...
Read the contents of this [Shiny App](https://beta.rstudioconnect.com/connect/#/apps/2692/access) upto just before "Shrinking $\beta$ Coefficients via LASSO" (which we will cover later).
...
* Due: Tue 10/3 9:00am, at which point there will be synchronized pull requests.
* Details:
    + Teams: Individual
    + Presentation: Done
    + Revision round: None
