...
* Final project discussions + Amy coming tomorrow to answer comps
* 
-->
...
fit/predict with logistic regression and answer these questions.

* After loading and wrangling the `profiles` data:
    + Create histograms of male and female heights using a bin width of 1 inch. IMO
there is an irregularity in the male heights histogram. What could be the cause
of it?
    + **Challenge question**: There are 24106 females and 35808 males in this dataset (OkCupid has
...
over height.
* Section 1.1.1: In code snippet `1.b)` in the resulting point-by-point
`augment()` table, why are there negative `.fitted` values?
* Section 1.1.2: Wrangle the data so that we have all four values needed to
...
* After looking at the [Wikipedia entry on confusion matrices](https://en.wikipedia.org/wiki/Sensitivity_and_specificity#Confusion_matrix) compute the:
