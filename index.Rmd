...
* Starting topic 2 today: Continuous outcomes part I
* We'll come back to Lecture 1.8 *Bias-variance tradeoff* later.


## Executive summary via `.gif`
...
# Let's join flight info for all 336K domestic flights that left an NYC airport
# in 2013 (JFK, EWR, or LGA) and join it with weather data:
View(flights)
View(weather)
...
flights_sample_test %>% 
  summarise(num_missing = sum(is.na(dep_delay_hat)))

flights_sample_test %>%
  filter(!is.na(dep_delay_hat)) %>% 
  summarise(RMSE = sqrt(mean((dep_delay - dep_delay_hat)^2)))
```
...
# Play around with the span argument here:
