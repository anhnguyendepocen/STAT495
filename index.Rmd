...
2.1: $\log()$ and $\log10()$ transformations.
...
```{r, fig.height=9/2, fig.width=16/2}
library(tidyverse)
...
Note that the two plots are identical, but the values on the axes are different.
...
# Let's join flight and join it with weather data after setting the seed
