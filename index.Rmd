...
$k$-means clustering problem set
...
Using the `k_means.R` available [here](), we can generate the following plot based
on our $k=10$ k-means clustering results:
...
* Cluster 3: Must be 8's
* Cluster 4: Likely 7's, but seems to be conflated with 4's and 9's
* Cluster 5: Must be 5's
* Cluster 6: Must be 3's
* Cluster 7: Must be 2's
* Cluster 8: Must be 1's. Again? Isn't that Cluster 1?
* Cluster 9: Must be 0's.
* Cluster 10: Could be 4's, 7's, and 9's again!

Remember:

* We are not modeling $\widehat{y} = \widehat{\mbox{digit}}$ a fitted digit
prediction, as we are doing unsupervised learning. In fact, no where in our
analyses is the information embedded in the human-tagged `label` variable being
used. In practice, you should be using this information!
* Certain true digits cluster themselves very nicely: 1 (twice), 6, 2, 8, 0 (twice)
* Note how the 4's, 7's, and 9's aren't that clear. 
