...
model_CART <- rpart(model_formula, data = iris, control=control)
...
arguments that control the shape of the tree in the `control` object above.
1. Create a new numerical outcome variable `iris$is_setosa <- as.numeric(iris$Species == "setosa")` and interpret the resulting tree.
...
following files should be modified/added:
    1. `PS08.Rmd`: Source code
    1. `PS08.html`: The output of `PS08.Rmd`.
    1. `README.md`: Change `rudeboybert` in the URL to your GitHub ID so that
    both Andrew and I can open your `PS08.html` in a browser.
    1. `submission.csv`
